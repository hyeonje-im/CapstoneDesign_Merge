...
class ScenarioManager:
    ...
    def on_sequence_complete(self, info=None):
        ...
        def _deferred_replan(grid_snapshot, res_snapshot):
            try:
                ...
                self._replan_requested = False
                self._replanning = False  # 재계획 완료 후 플래그 해제

        # 1.0초 지연 타이머 시작
        print(f"[Scenario] 경로 완료. 다음 계획까지 1.0초 대기...")
        grid_now = self.get_grid() # 현재 그리드 상태를 캡처
    ...
    def get_mode_ui_state(self, drain_new=False):
        """
        현재 모드가 UI에 전달해야 하는 상태 딕셔너리를 반환한다.
        drain_new=True일 경우 새로운 상태를 한번만 가져오고 비운다.
        (RestaurantMode, RandomMode 등에서 사용)
        """
        if not hasattr(self, "_ui_state_buffer"):
            self._ui_state_buffer = {}

        if drain_new:
            state = self._ui_state_buffer.copy()
            self._ui_state_buffer.clear()
            return state

        return self._ui_state_buffer.copy()
