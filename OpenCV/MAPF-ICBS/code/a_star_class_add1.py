...
COST_STRAIGHT = 1.2
COST_TURN = 1.6
COST_MIN_MOVE = min(COST_STRAIGHT, COST_TURN)
...
class A_Star(object):
    ...
    def __init__(self,my_map,starts,goals,heuristics,agents,contraints,yield_prev_table=None):
        ...
        self.agents = agents
        self.yield_prev_table = yield_prev_table
    ...
    def generate_child_nodes(self, curr):
        ...
        for dirs in ma_dirs: 
            ...
            h_value = sum(self.heuristics[i][child_loc[i]] for i in range(len(self.agents))) * COST_MIN_MOVE
            ...
            for i in range(len(self.agents)):
                if not curr['reached_goal'][i]:
                    prev_prev = curr['parent']['loc'][i] if curr.get('parent') else None
                    prev = curr['loc'][i]
                    nxt = child_loc[i]
                    st = _step_type(prev_prev, prev, nxt)
                    if st == "wait":
                        g_inc += COST_WAIT
                    elif st == "straight":
                        g_inc += COST_STRAIGHT
                    ...
                    if self.yield_prev_table is not None:
                        t_next = curr['timestep'] + 1
                        cells = self.yield_prev_table.get(t_next)
                        if cells is not None and nxt in cells:
                            g_inc += PEN_YIELD

            g_value = curr['g_val'] + g_inc
    ...
    def find_paths(self):
        ...
        h_value = sum(self.heuristics[i][self.starts[i]] for i in range(len(self.agents))) * COST_MIN_MOVE
